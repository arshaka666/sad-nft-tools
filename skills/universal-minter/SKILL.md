---
name: universal-minter
description: OpenClaw SuperAgent v4 — universal NFT minter, OpenSea/Manifold/Zora/Seadrop, auto-gas
category: openclaw-v4
---
...
## Signature-Gated & Two-Step Mint Patterns (v4+)

Not all mints are one-shot `mint(uint256)`. Some use **commit-then-reveal** with an off-chain signature from a trusted signer. If all standard mint functions return empty revert (`'0x'`): check the ABI for two-step patterns.

### Pattern: commitToMint + mintWithDice (dice roll)

```
1. commitToMint() [payable] → bayar commitFee, emit event dgn diceResult + nonce
2. mintWithDice(diceResult, nonce, signature) [payable] → perlu signature trusted signer
```

**What to check first:**
- `commitFee()` → biaya commit (usually fixed small ETH)
- `mintPrice()` → bisa 0 (free mint after commit)
- `mintState(address)` → lihat status commit si wallet (struct: committed, diceResult, nonce)
- `setTrustedSigner()` / `trustedSigner()` → siapa yg sign

**Reality check:** tanpa private key trusted signer, `mintWithDice()` gak bisa di-execute langsung dari script. Butuh frontend dApp atau API yg generate signature. Jika user punya akses ke frontend-nya, coba commit dulu, inspect event, lalu cari signature dari frontend API / Etherscan internal tx.

### Other two-step patterns

| Pattern | Functions | Notes |
|---------|-----------|-------|
| Thirdweb Signature Drop | `claimWithSignature(address, ClaimRequest, bytes)` | Signature dari key yg authorized. Check `signer()` or `verify()` |
| EIP-712 Allowlist | `mintWithAllowlist(uint256, bytes32[], bytes)` | Merkle proof-based, signature-based |
| Commit → Reveal | A: `commit(bytes32)` → B: `reveal(uint256, bytes32)` | Hash-based, no signature needed — doable on-chain |
| Lazy mint + Claim | `claim(address, uint256, bytes32[], bytes)` | Manifold/thirdweb pattern, needs proof or signature |

### Detection flowchart

```
All standard mint() fns revert empty?
  │
  ├─ Check ABI (Blockscout / Etherscan)
  │   If no ABI: try ERC721 / ERC1155 balanceOf to confirm token type
  │
  ├─ commitToMint exists?
  │   → dice-roll mint (signature-gated)
  │   → warn user: perlu akses ke trusted signer
  │
  ├─ commit(bytes32) exists?
  │   → hash-based commit-reveal (no signature!)
  │   → bisa di-execute on-chain
  │
  ├─ claimWithSignature / mintWithAllowlist exists?
  │   → signature/merkle-gated
  │   → cek berapa banyak minted vs MAX_SUPPLY
  │
  └─ None of the above
      → mint mungkin belum active atau udah closed
      → cek saleActive() / isActive() / paused()
```
...
- **revert no reason / empty '0x'** → ABI contract likely uses custom mint flow (Zora drop, Sound, custom Foundation, or **signature-gated two-step mint**) → manual ABI inspection needed via Blockscout/Etherscan
