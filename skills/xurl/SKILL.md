...
### NFT radar Telegram formatting

For the user's X/Twitter NFT radar cron posts, use the formatting and routing conventions in `references/nft-radar-telegram-format.md`. Key points: keep WL/Allowlist/Whitelist/Early Access separate from FREE/STEALTH/LIVE mint, use mobile-readable spacing, extract project handle/title, requirements or mint highlights, concise cleaned text, and a single horizontal `[X Post](...)` link.
