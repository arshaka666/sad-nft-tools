...
def _authorize_bridge_tx(w3: Web3, account: LocalAccount, tx: dict,
...
async def lifi_bridge(w3: Web3, account: LocalAccount,
                       ...
                       amount: int, to_address: Optional[str] = None,
                       execute: bool = False,
                       usd_value: Optional[float] = None,
                       slippage: float = 0.01) -> BridgeResult:
    ...
    decision, gov, intent = _authorize_bridge_tx(w3, account, tx,
                                                  usd_value=usd_value,
                                                  slippage_pct=slippage * 100)
    if not decision.allowed:
        return BridgeResult(status=decision.verdict, error=decision.summary())
    if not execute:
...
async def stargate_v2_bridge(w3: Web3, account: LocalAccount,
                              ...
                              underlying_token_addr: Optional[str] = None,
                              execute: bool = False,
                              usd_value: Optional[float] = None) -> BridgeResult:
