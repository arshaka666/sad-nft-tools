...
async def task_swap_loop(wallet: WalletRef, amount: float,
    ...
    result = await swap_evm(w3, account, base_token, quote_token, amount_wei,
                            slippage_pct=1.0, execute=execute)
    return result
...
async def task_daily_checkin(wallet: WalletRef, contract: str,
    ...
    """Call contract.checkIn() through ContractWriter; dry-run/gated unless execute=True."""
    from eth_account import Account
    import os
    ...
    abi = [{"name": "checkIn", "inputs": [], "type": "function", "stateMutability": "nonpayable"}]
    writer = ContractWriter(contract, int(wallet.chain) if wallet.chain.isdigit() else 1,
                            account, abi=abi, rpc_url=rpc)
    if not execute:
        tx = writer.build_tx("checkIn")
        ok, reason = writer.simulate(tx)
        return {"status": "dry_run", "simulated": ok, "detail": reason, "tx_hash": None}
    result = await writer.call("checkIn", usd_value=usd_value)
    return {"status": result.status, "detail": result.detail, "tx_hash": result.tx_hash}
