...
def _authorize_tx(w3: Web3, account: LocalAccount, action: str, tx: dict,
    ...
    """Run SpendGovernor before any EVM broadcast."""
...
def approve_if_needed(w3: Web3, account: LocalAccount, token: str,
                      spender: str, amount: int, use_exact=True,
                      *, execute: bool = False,
                      usd_value: Optional[float] = None) -> Optional[str]:
    ...
    if approve_amount == 2**256 - 1:
        raise RuntimeError("refusing infinite approval; use exact approval only")
    ...
    decision, gov, intent = _authorize_tx(w3, account, "approve", tx, usd_value=usd_value)
...
async def swap_evm(
    ...
    if token_in.lower() != NATIVE_SENTINEL_EVM.lower():
        ...
        approve_if_needed(w3, account, token_in, spender, amount_wei,
                          execute=execute, usd_value=usd_value)
    ...
    decision, gov, intent = _authorize_tx(w3, account, "swap", tx,
                                           usd_value=usd_value,
                                           slippage_pct=slippage_pct)
    if not decision.allowed:
        return SwapResult(status=decision.verdict, warnings=warnings,
                          error=decision.summary())
    if not execute:
        return SwapResult(status="dry_run", warnings=warnings,
...
async def sell_token_full(w3: Web3, account: LocalAccount, token_address: str,
                           ...
                           oneinch_api_key: Optional[str] = None,
                           execute: bool = False,
                           usd_value: Optional[float] = None) -> SwapResult:
    ...
    return await swap_evm(w3, account, token_address, NATIVE_SENTINEL_EVM,
                           ...
                           skip_safety=True, execute=execute,
                           usd_value=usd_value)  # sell sendiri, gak perlu honeypot check
...
async def swap_solana(client, keypair, input_mint: str, output_mint: str,
    ...
    if not execute:
        return SwapResult(status="dry_run", error="dry run only; pass execute=True after user confirmation")
