...
### Batch Balance Check (banyak wallet)

Ketika user minta cek balance 10+ wallet sekaligus, **tulis script ke file dulu** — bukan inline terminal. Ini menghindari masalah nested f-string escaping.

```python
# /tmp/check_batch.py — write via write_file, run via terminal
from web3 import Web3

w3 = Web3(Web3.HTTPProvider('https://eth.drpc.org'))

wallets = {4: "0x16472f76B26FfC61eB41B747a66D2e9FDf7611C7", 5: "0xA1D97d40aDDFB6D51eca140cF1e75Ab41f5cAa64"}

total = 0
for num, addr_str in sorted(wallets.items()):
    addr = Web3.to_checksum_address(addr_str)
    bal = w3.eth.get_balance(addr)
    eth_val = w3.from_wei(bal, 'ether')
    total += eth_val
    print(f"W{num}: {eth_val:.6f} ETH  {'✅' if eth_val > 0 else '❌'}")
print(f"Total: {total:.6f} ETH")
```

**Tips untuk 50+ wallet:**
- Bagi per batch 10-20 wallet per script (hindari rate-limit RPC)
- Public RPC (`eth.drpc.org`) cukup untuk balance check sederhana
- 70+ wallet → ~30-60 detik total
- Simpan address di file pusat (`ALL_WALLETS_unified.txt`) dan baca dari situ
