...
| Native Web3 slash commands (`/wallet`, `/portofolio`, `/track-wallet`, `/mint-history`) or batch-create encrypted Ethereum/NFT wallets | `references/native-slash-and-wallet-vault.md` |
...
| Mint NFT — direct contract, OpenSea Seaport drop (GraphQL zun2025), hammer/poll auto-mint | `references/minting.md` |
...
| **Telegram forum/topic delivery for NFT/Web3 research updates** — discover live topic targets, avoid `Topic_closed`, and keep notifications secret-free | `references/telegram-forum-research-delivery.md` |
...
Hermes selalu kembalikan output yang **ringkas & rapi** — tanpa baris kosong berlebihan, tanpa spasi sia-sia, dan tanpa placeholder/command mentah seperti `$(cat file)` di pesan user-facing. Format tabel pake pipe (jika perlu), baris antara section minimal.

### NFT/Web3 Research Telegram Delivery

Untuk update radar/research NFT, terutama hasil monitoring X/Twitter atau Waypoint/live mint cron, pastikan pesan siap kirim ke Telegram: judul jelas, trigger/signal, ringkasan, risk note, dan next step. Jangan kirim output shell mentah, placeholder, path lokal, credential, token, seed, private key, atau konten file credential. Ikuti routing detail dan anti-spam rules di `references/telegram-forum-research-delivery.md` sebelum mengirim ke forum topic.

User sangat sensitif terhadap spam/duplicate radar: update harus compact seperti pinned note, pakai WIB, jangan raw URL panjang, jangan tampilkan wrapper cron/job metadata, jangan ulang collection/posting yang sama, dan prioritaskan collection/postingan bulan ini yang baru saja aktif.

1. **Plan ringkas** (1–3 baris, tanpa enter berlebih)
2. **Konfirmasi user** (kecuali user sudah set `auto_confirm=True`)
3. **Hasil**: tx hash, link explorer, status, gas/fee
4. **Saran lanjutan** kalau relevan

Untuk batch operations, return tabel: wallet_address | action | status | tx_hash.

**Hindari:** enter/gap berlebih antar baris, spasi yang gak perlu, format yang terlalu renggang.
